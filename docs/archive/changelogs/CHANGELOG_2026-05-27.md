...
## 변경 이력 (10부) — 엔진 vs 도메인 경계, 그리고 CT/MRI 볼륨 로딩

> 이 항목은 "기능 추가"보다 **설계 결정**의 기록이다. 실제 CT/MRI 파일 로딩에
> 들어가기 전, 사용자가 던진 고민에서 출발한다.

### 37. 고민 — 범용 엔진에 도메인 케이스가 박히는 것

사용자 우려(원문 취지): *"Mini-Engine은 범용 엔진인데 자율주행·의료영상 같은 좁은
도메인 케이스가 엔진에 구현되면, 이 엔진으로 제품을 만들 때 너무 무거워지는 것
아닌가?"*

타당한 직감이다. 그대로 두면 엔진 코어에 DICOM 파서, 센서 물리 모델, 도시 씬
같은 도메인 코드가 쌓여 (1) 빌드/바이너리가 비대해지고 (2) 관심사가 섞여 "범용
엔진"이라는 명제가 무너진다.

### 38. 해결 원칙 — 능력(capability)은 코어, 데이터/정책(domain)은 그 위

상용 엔진의 표준 분리를 적용: **엔진 코어는 범용 능력만, 도메인은 그 위의 앱·샘플·
옵션 모듈**. 판단 기준은 *"이것이 능력인가, 데이터/정책인가"*.

이 기준으로 본 현 상태:

| 항목 | 분류 | 위치 |
| --- | --- | --- |
| 볼륨 렌더링(3D 텍스처·레이마칭·뎁스 합성·TF LUT) | 범용 능력 | 엔진 코어 ✅ (게임 구름·연기, 의료, 과학 시각화 공통) |
| CT-Bone/Soft 프리셋 **값** | 도메인 데이터(작음) | 경계 — LUT control point일 뿐이라 부담 미미 |
| **raw/DICOM 파일 로더** | 도메인(포맷 무거움) | **엔진 밖 — 앱/옵션 모듈** |
| 도시 씬·WorldManager·센서 | 도메인(애플리케이션) | 이미 앱 레이어 |

→ 현재 코어에 든 건 대부분 범용 능력이고, "무거워질" 위험은 도메인 로더/데이터다.
그건 **링크 단위로 분리**하면 안 쓰는 제품에선 빠진다.

### 39. 기술 — `loadFromData` seam

핵심은 엔진과 데이터 소스 사이에 **단 하나의 seam**을 긋는 것:

- 엔진(`VolumeRenderer`)은 **`loadFromData(buffer, w, h, d)`** 하나만 노출 —
  "임의 차원 R8 밀도 버퍼를 받아 렌더". 절차적 생성기는 이 seam 위의 **한 소스**가
  됐다(기존 `uploadVolume`을 임의 차원으로 일반화, `initialize`가 절차적 → loadFromData).
- 도메인 로더는 `src/assets/VolumeFile`(**앱 레이어, 네이티브**)에 둠 — headerless
  raw를 8/16비트로 읽어(16비트 LE는 min/max 정규화→8비트) 버퍼를 만들고 seam에
  넘긴다. **엔진은 "의료"를 전혀 모른다.** 안 쓰면 안 링크된다(빌드에서 분리).
- 런타임 안전: `loadFromData`가 텍스처/뷰 재생성 + (저장해둔 depth view로)
  바인드그룹 재빌드까지 처리 — 리사이즈 처리와 같은 패턴.
- 트리거(데모용): `MINIENGINE_VOLUME="path,W,H,D[,bytesPerVoxel]"` 환경변수 →
  `assets::loadRawVolume` → `volumeRenderer->loadFromData`. 없으면 절차적(현행).

### 40. 검증

실제 CT가 없어 합성 96³ 구체 `.raw`를 만들어 라운드트립 확인: 로더가 파일을 읽고
(`VolumeFile loaded ... 96x96x96`), 엔진이 비큐빅 텍스처로 업로드
(`VolumeRenderer loaded volume data 96x96x96`), 검증 에러 0, 구체 볼륨 렌더(사용자
확인). 절차적 경로 무회귀. → 실제 CT/MRI raw(uint8/uint16-LE)는 차원만 주면 그대로
로드된다.
...
| 파일 | 변경 |
| --- | --- |
| `src/rendering/VolumeRenderer.{hpp,cpp}` | `loadFromData` seam; `uploadVolume`를 임의 차원(w,h,d)으로 일반화; depth view 저장 후 reload 시 바인드그룹 재빌드 |
| `src/assets/VolumeFile.{hpp,cpp}` | **신규(앱 레이어)** — headerless raw 볼륨 로더(uint8 / uint16-LE 정규화), 파일 없으면 graceful false |
| `src/rendering/Renderer.cpp` | `MINIENGINE_VOLUME` 환경변수 파싱 → 로더 → `loadFromData`(네이티브); 없으면 절차적 |
| `CMakeLists.txt` | 네이티브 타깃에 `VolumeFile.cpp`(앱 레이어 로더) |

### 42. 의미

엔진/앱 경계를 **코드로 강제**한 사례 — "능력은 엔진, 도메인은 그 위"를 seam 하나로
구현. 면접 포인트: *"범용 엔진이 의료 시각화도 하지만, 엔진 코어엔 의료 코드가
없다 — 3D 텍스처를 받는 seam 하나뿐이고, CT 로더는 안 링크하면 빠지는 앱 레이어
모듈"*. 향후 후속: 16비트 CT 정규화 고도화(윈도우/레벨), WASM 볼륨 프리로드,
DICOM/NIfTI 임포터(역시 앱 레이어), `*.raw`는 대용량이라 커밋 제외(.gitignore 권장).
