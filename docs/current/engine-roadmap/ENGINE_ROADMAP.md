...
- **D4** — ✅ 검증(경쟁/깜빡임 없음) + CPU 기록 시간 측정. A/B 토글 + ImGui 표시.
  **정직한 결론**: 셰도우는 단일 인스턴스드 드로우라 캐스케이드당 기록이 O(1)(~6 명령)
  → 병렬이 thread 오버헤드(~0.02ms)만큼 오히려 느림(기록-바운드 아님). 병렬 이득은
  패스당 수백 개 비인스턴스 드로우일 때만. 능력을 만들고 측정해 "언제 이득인지"를
  정량 규명. 자세한 내용: [`CHANGELOG_2026-05-27.md`](../../archive/changelogs/CHANGELOG_2026-05-27.md) 4부.

**로드맵 D 종결** (D1~D4, 2026-05-27).
