...
fn sampleVolume(uvw: vec3<f32>) -> f32 {
    ...
    // Last-brick shrink follow-up: for LOD 0 the atlas texture size is the
    // CPU-computed physical dim (last brick may drop its outer halo + shrink
    // interior). LOD 1..3 keep the uniform atlasG * stored formula.
